...
void testRealMap(void) {
    ...
    std::map<int, int>::iterator ite = mp2.end();
    ite--;

    std::map<int, int>::reverse_iterator it1 = mp2.rbegin();
    std::map<int, int>::reverse_iterator it2 = mp2.rend();
    it2--;
    if (it1->first == ite->first && it->first == it2->first) {
        std::cout << "Tested end" << std::endl << "Tested rbegin" << std::endl;
        std::cout << "Tested rend" << std::endl;
    ...
    mp.insert(mp.end(), std::pair<int, int>(5, 5));
    it = mp.begin();
    ite = mp.end();
    ite--;
    if (it->first == 4 && ite->first == 5)
    ...
    std::map<int, int>::iterator itLow = mp.lower_bound(3);
    std::map<int, int>::iterator itUp = mp.upper_bound(3);
    mp.erase(itLow, itUp);

    it = mp.begin();
    std::map<int, int>::const_iterator iter = mp.begin();
    if (iter == mp.begin())
        ;
